...
for model_path in path_list:
    ...
    # Reiniciar la variable de simulación para cada iteración
    experimet = None 
    ...
    try:                                 
        ...
        # B. APLICACIÓN DEL MEDIO (Simplificado por brevedad, asumo que las 40+ líneas son correctas)
        ...
        if experimet is not None and experimet.total_biomass is not None:
             status = "ÉXITO/REGISTRO"
        else:
             status = "FALLO CRÍTICO DE SIMULACIÓN"
             break

        print(f"--- Iteración para {model_id} finalizada. Estado: {status} ---")
...
# Para ver solo los que fueron viables (asumiendo que 'model_summary_data' ya fue llenado)
modelos_viables = [item['ID'] for item in model_summary_data]
print("\nModelos que simularon exitosamente (Viables):")
print(modelos_viables)
